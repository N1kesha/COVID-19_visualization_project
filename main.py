...
with st.echo(code_location='below'):
    ...
    df = df.drop(
        ["total_vaccinations", "new_cases", "new_deaths", "total_boosters", "new_vaccinations", "people_vaccinated"],
        axis=1)
    ...
    triple = []
    for country in countries:
        triple.append([country, df[df['location'] == country]['continent'].unique()[0],
                       df[df['location'] == country]['slope'].unique()[0]])
    ...
    for country in countries_for_vaccines:
        list_perc_of_deaths_and_vaccines.append([country,
                                                 100 * int(
                                                     df[df['location'].isin([country])].tail(1)[
                                                         'total_deaths']) / float(
    ...
    mortality_and_vaccinated_df = pd.DataFrame(list_perc_of_deaths_and_vaccines,
                                               columns=['country', 'perc. of mortality', 'perc. of vacc',
                                                        'perc. of morbidity'])
    ...
    ### FROM: "https://www.youtube.com/watch?v=GyMO9WCEheQ&t=170s"
